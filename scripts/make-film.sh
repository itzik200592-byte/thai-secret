...
case "$ORI" in
  pc)     AR=16:9; W=1280; H=720;  FILM="film.mp4";   POSTER="poster.jpg";   SUF="" ;;
  mobile) AR=9:16; W=720;  H=1280; FILM="film_m.mp4"; POSTER="poster_m.jpg"; SUF="_m" ;;
...
# ---- the film: 5 clips x 5 s = 25 s. Same fixed GRADE clause in every prompt. ----
...
# Keyframes K0..K5 (K_i = end of clip i = start of clip i+1)
declare -A KF
...
KF[3]="close-up of exactly two hands of $MODEL working thick glossy white cream through the mid-lengths and ends of her long dark hair, strands coated and clumping smoothly, hair held over the white marble counter, anatomically correct, $SALON blurred, $FRAMING, $GRADE"
KF[4]="close-up at a white salon wash basin: $MODEL leans back, her long dark hair wet and smooth, clean warm water from a handheld sprayer runs over the hair and her hand glides through it easily, exactly two hands visible, anatomically correct, bright white salon, $FRAMING, $GRADE"
KF[5]="medium shot: $MODEL faces the camera with a slight smile in $SALON, her long dark hair now smooth, glossy and heavy, soft window light sliding along it like silk, $FRAMING, $GRADE"
...
MV[3]="slow drift along the hair length as her hands work the white cream through the mid-lengths and ends, the strands clump smoothly and the cream melts in"
MV[4]="camera rises slowly with the water at the salon basin, warm water rinses the cream out, her hand glides through the wet hair without snagging"
MV[5]="she lifts her head and turns toward camera, hair now smooth glossy and heavy, light slides along it, camera settles and holds still"

echo "== [$ORI] phase 1: keyframes"
for i in 0 1 2 3 4 5; do
  if [ -f "$K/K$i.png" ]; then echo "have K$i"; continue; fi
  extra="$REFS_MODEL"; [[ "$i" =~ ^[123]$ ]] && extra="$REFS_MODEL $REFS_SACHET"
...
echo "== [$ORI] phase 3: assemble $FILM (dense keyframes for scrubbing)"
f=""; for i in 1 2 3 4 5; do f="$f -i $C/c$i.mp4"; done
ffmpeg -y -loglevel error $f -filter_complex "[0:v]fps=24,scale=$W:$H,setsar=1[v0];[1:v]fps=24,scale=$W:$H,setsar=1[v1];[2:v]fps=24,scale=$W:$H,setsar=1[v2];[3:v]fps=24,scale=$W:$H,setsar=1[v3];[4:v]fps=24,scale=$W:$H,setsar=1[v4];[v0][v1][v2][v3][v4]concat=n=5:v=1:a=0[v]" \
...
echo "DUR (seconds per clip, paste into Home.jsx):"
for i in 1 2 3 4 5; do ffprobe -v error -show_entries format=duration -of csv=p=0 "$C/c$i.mp4"; done
